...
export default function SearchableSelectInput({
  ...
  const customStyles: StylesConfig = {
    control: (styles) => ({
      ...
      transition: 'none',
      '&:focus-within': {
        border: `1px solid ${error ? '#EF4444' : 'var(--color-primary-600)'}`,
        boxShadow: `0 0 0 1px ${
          error ? '#EF4444' : 'var(--color-primary-600)'
        }`,
      },
      '*': {
    ...
    option: (styles, state) => ({
      ...styles,
      color: 'black',
      background: state.isSelected ? 'rgba(255, 230, 109, 0.30)' : 'white',
      ':hover': {
        background: 'rgba(255, 230, 109, 0.50)',
      },
      cursor: 'pointer',
    }),
    multiValue: (styles) => ({
